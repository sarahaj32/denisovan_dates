wildcard_constraints:
    rep="\d+",
    frag_source="(sim_intro|hmmix.100|hmmix.500)",
...
rule sims_all:
    input:
        ...
        # using all positions:
        #expand("simdat/{sim}_{seed}/DEN/curve/results/{sim}_{source}_{rep}_D.txt", rep = list(range(1,21)), sim = ["denisovan_simple", "denisovan_early", "denisovan_late", "denisovan_low"], source = ["sim_intro", "hmmix.500"], seed = [1,2,3]), # "", "_downsampled",  
        # using ingroup positions only:
        #expand("simdat/{sim}_{seed}/DEN/curve/results/{sim}_{source}_{nafr}Afr_ingroup_{rep}_D.txt", rep = list(range(1,21)), sim = ["denisovan_simple", "denisovan_early", "denisovan_late", "denisovan_low"], source = ["sim_intro", "hmmix.500"], seed = [1,2,3], nafr = [500]), # "", "_downsampled",  )
        #expand("simdat/{sim}_{seed}/DEN/curve/results/{sim}_{source}_{nafr}Afr_ingroup_{rep}_D.txt", rep = list(range(1,21)), sim = ["denisovan_simple", "denisovan_early", "denisovan_late", "denisovan_low"], source = ["sim_intro", "hmmix.100"], seed = [1,2,3], nafr = [100]), # "", "_downsampled",  )
        
        # using 
        # SARAH THESE ARE WHAT I WANT!!!!
        ...
        expand("simdat/{sim}_{seed}/DEN/fragments/{sim}_sim_intro_{rep}.bed", rep = list(range(1,21)), sim = ["denisovan_simple", "denisovan_LS_ice", "denisovan_LS_pap"], seed = [1]), # increase to 500???? , "hmmix_LS" , "denisovan_simpleLS" "denisovan_smallAfr", "denisovan_bigNonAfr",
        expand("simdat/{sim}_{seed}/DEN/fragments/{sim}_hmmix.{nafr}_{rep}.bed", rep = list(range(1,21)), sim = ["denisovan_simple", "denisovan_LS_ice", "denisovan_LS_pap"], nafr = [100, 200, 500], seed = [1]), # increase to 500???? , "hmmix_LS" , "denisovan_simpleLS" "denisovan_smallAfr", "denisovan_bigNonAfr",
...
# produce a vcf of the populations of interest, as well as saves the tree file from the simulation (so we can access and compute on it later)
    ...
    shell: 
        ...
        """
        python {input.script} -j {input.json} -c {wildcards.rep} -y {input.deme} -o {output.vcf} -g 29 -l 100000000 -m 1.29e-8 -r 1e-8 -t {output.tree} -s {wildcards.seed}
...
rule remove_recurring_mutations:
    ...
    shell:
        "bcftools view -c 1 {input} | bcftools norm -m + | bcftools view -M 2 > {output}"
...
rule extract_archaics:
    ...
    input: 
        "simdat/{sim}_{seed}/clean/{sim}_sim_{rep}.vcf"
    output:
        "simdat/{sim}_{seed}/archaics/{sim}_sim_{rep}.vcf"
    shell:
        "bcftools view -c1 -s AltaiNeandertal,Vindija33.19,Chagyrskaya-Phalanx,Denisova {input} -o {output}"

rule annotate_archaic_variants:
    input:
        archaics="simdat/{sim}_{seed}/archaics/{sim}_sim_{rep}.vcf",
        script="add_annotations_to_archaic_observations.py"
    output:
        annotated="simdat/{sim}_{seed}/archaics/{sim}_annotatedArchaics_{rep}.txt"
    shell:
...
rule collate_frags:
    ...
    output:
        tmp = temp("simdat/{sim}_{seed}/DEN/fragments/{sim}_{method}.{nafr}_all.bed"),
        chrs = expand("simdat/{{sim}}_{{seed}}/DEN/fragments/{{sim}}_{{method}}.{{nafr}}_{rep}.bed", rep = list(range(1,21)))
    shell:
        """
        rm -f {output.tmp}
        hap_count=0
...
rule create_ancestry_matrix: # this is for hmmix or true ancestry, but using all of the variable positions in that populatio of interest
    input:
